...
describe("transformRequestBody — general", () => {
  it("returns undefined unchanged", () => {
    expect(
      transformRequestBody(undefined, { format: "openai" }),
    ).toBeUndefined();
  });
  ...
  it("returns original body when JSON is not an object", () => {
    expect(transformRequestBody("[1,2,3]", { format: "openai" })).toBe(
      "[1,2,3]",
    );
    expect(transformRequestBody('"hello"', { format: "openai" })).toBe(
      '"hello"',
    );
  });
...
describe("transformRequestBody — cache_control (Anthropic)", () => {
  it("adds cache_control to last user message with string content", () => {
    ...
    // Earlier messages also converted to array content (gateway requires array content)
  ...
  it("does NOT overwrite existing cache_control on last block", () => {
    ...
    const body = JSON.stringify({
      messages: [
        {
          role: "user",
          content: [{ type: "text", text: "x", cache_control: existing }],
        },
      ],
    });
    const out = transformRequestBody(body, { format: "anthropic" });
...
describe("transformRequestBody — start-plan system (Anthropic)", () => {
  it("prepends current ZCode system messages as three cacheable blocks", () => {
    ...
    expect(parsed.system[2].text).toContain(
...
describe("transformRequestBody — start-plan system (OpenAI)", () => {
  it("prepends current ZCode system messages before OpenAI chat messages", () => {
    ...
    expect(parsed.messages[2].content).toContain(
      "You have been invoked in the following environment:",
    );
...
describe("transformRequestBody — metadata.user_id (Anthropic)", () => {
  ...
  it("does NOT touch body when metadata.user_id already equals ctx.userId", () => {
    ...
    expect(
      transformRequestBody(body, { format: "anthropic", userId: "u_x" }),
    ).toBe(body);
  });
