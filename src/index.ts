...
function main(): void {
  ...
  if (cmd === "auth") {
    ...
    const serveArgs =
      cmd === "serve" ? parseServeArgs(args.slice(1)) : parseServeArgs(args);
...
async function serve(
  ...
  if (!existsSync(path)) {
    ...
    console.log(
      `Edit auth.apiKey, or run: zcode-proxy auth login <zai|bigmodel>\n`,
    );
  }
  ...
  if (config.auth.mode === "oauth") {
    ...
    if (!cred) {
      console.error(
        "Not logged in. Run: zcode-proxy auth login " + config.provider,
      );
      process.exit(1);
    }
...
function printDebugBanner(config: ProxyConfig, path: string): void {
  const cred =
    config.providers[config.provider].credential ?? config.auth.apiKey;
  const credShape = cred
    ? `${cred.slice(0, 6)}...${cred.slice(-4)} (${cred.length} chars)`
    : "(none — oauth)";
  ...
  console.log(
    `  proxy api key: ${config.auth.proxyApiKey ? "required" : "open (no client auth)"}`,
  );
  console.log(`  provider: ${config.provider}`);
  console.log(`  plan: ${config.plan}`);
  console.log(
    `  identity: appVersion=${config.identity.appVersion} sourceTitle=${config.identity.sourceTitle} referer=${config.identity.refererOrigin}`,
  );
  console.log(
  ...
  console.log(
    `  models (${config.models.length}): ${config.models.join(", ")}`,
  );
...
async function authLogin(args: string[]): Promise<void> {
  ...
  console.log(
    `Logging in: ${provider}${importMode ? " (import)" : " (OAuth)"}\n`,
  );
...
function importFromZCodeConfig(provider: ProviderId): Credential {
  ...
  try {
    ...
    console.error(
      "Make sure ZCode is installed and you've logged in at least once.",
    );
    process.exit(1);
  }
  ...
  const jwt =
    config.provider?.[startPlanKey]?.options?.apiKey?.trim() || undefined;
