...
export function translateRequestOpenAIToAnthropic(
  ...
  const anthropicMessages =
    translateMessagesWithToolCoalescing(nonSystemMessages);

  const result: AnthropicMessagesRequest = {
    model: req.model,
    messages: anthropicMessages,
    max_tokens:
      req.max_tokens ?? req.max_completion_tokens ?? DEFAULT_MAX_TOKENS,
  };
...
function translateThinking(
    ...
    if (explicit.type === "enabled" || explicit.type === "adaptive") {
      ...
      return {
        ...
        ...(typeof budget === "number" && Number.isFinite(budget) && budget > 0
          ...
          : {}),
        ...(explicit.type === "adaptive" &&
        typeof explicit.display === "boolean"
  ...
  if (isReasoningModel(req.model)) {
    // Anthropic spec requires budget_tokens when thinking.type === "enabled".
    // Default to a reasonable budget that fits within max_tokens.
    const maxTokens =
      req.max_tokens ?? req.max_completion_tokens ?? DEFAULT_MAX_TOKENS;
    const budget = Math.min(Math.max(Math.floor(maxTokens * 0.8), 1024), 10000);
 ...
 * tool results).
 */
function translateMessagesWithToolCoalescing(
...
function translateMessageOpenAIToAnthropic(
  msg: OpenAIMessage,
): AnthropicMessage {
...
export function translateResponseAnthropicToOpenAI(
  ...
  const reasoningContent =
    thinkingBlocks.map((b) => (b as any).thinking ?? "").join("") || undefined;
  const toolCalls =
...
function translateContentOpenAIToAnthropic(
  msg: OpenAIMessage,
): string | AnthropicContentBlock[] {
  if (typeof msg.content === "string") return msg.content;
...
function translateToolOpenAIToAnthropic(
